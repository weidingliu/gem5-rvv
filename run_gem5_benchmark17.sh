...
GEM5_DIR=/home/liuweiding/gem5-rvv              # Install location of gem5
...
if [[ "${BENCHMARK:0-1:1}" == "r" ]]; then
    RUN_DIR=$SPEC_DIR/benchspec/CPU/$BENCHMARK_CODE/run/run_base_refrate\_rvv-64.0000     # Run directory for the selected SPEC benchmark
fi
if [[ "${BENCHMARK:0-1:1}" == "s" ]]; then
    RUN_DIR=$SPEC_DIR/benchspec/CPU/$BENCHMARK_CODE/run/run_base_refspeed\_rvv-64.0000     # Run directory for the selected SPEC benchmark
fi
...
$GEM5_DIR/configs/spec_config/se_spec17.py \
$BOOM_FLAGS \
--benchmark=$BENCHMARK \
